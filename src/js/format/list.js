...
class OrderedList extends Container {
  ...
  optimize () {
    ...
    Array.from(this.domNode.childNodes).reduce((data, child) => {
      if (!UnorderedList.allowedChilren.some(value => child.__detail instanceof value)) {
...
UnorderedList.allowedChilren = [ListItem];
OrderedList.allowedChilren = [ListItem];
