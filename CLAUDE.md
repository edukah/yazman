...
```bash
npm run dev      # Webpack dev server on localhost:9004 (or https://yazman.dev:9004 with local SSL)
npm run build    # Production build → dist/yazman.min.js + dist/yazman.min.css
npm run release  # Build + copy to docs/assets/
```

`@web/test-runner` is configured. Run tests with `npm test` or `npm run test:watch` for watch mode.
